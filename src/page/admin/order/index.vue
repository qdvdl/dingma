<template>
  <div>
      <div class="search-box">
        <el-form :inline="true" :model="extra" class="demo-form-inline">
            ...
            <el-form-item label="订单状态:">
              <el-select @change="select_chg" v-model="extra.deal_state" placeholder="请选择">
            ...
            <el-form-item label="车辆品牌:">
              <el-select @change="select_chg" v-model="extra.brand_id" placeholder="请选择">
                 <el-option label="全部" value=""></el-option>
                 <el-option v-for="a in admin" :key="a.value" :label="a.text" :value="a.value"></el-option>
              </el-select>
            </el-form-item>

            <el-form-item label="车辆型号:">
              <el-select  @change="select_chg" v-model="extra.vehicle" placeholder="请选择">
                  <el-option label="全部" value=""></el-option>
                 <el-option v-for="v in vehicle" :key="v.value" :label="v.text" :value="v.value"></el-option>
              </el-select>
            </el-form-item>
      ...
      <div class="footerText">
        ...
        <strong>成交车辆数量:</strong><span>{{dealv_sum}} 辆</span>
      </div>
      <ImportExport :isImport="isImport" :isExport="isExport" :isDown="isDown" @successLoad='successLoad' :extra="extra" :exportUrl="exportUrl" :templateUrl="templateUrl" :action="action">
       <div slot="btnheader">
         <el-button type="primary" size="small" round  @click="eventOperation({data:{},type:'add'})">添加订单<i class="el-icon-plus el-icon--right"></i></el-button>
       </div>
      </ImportExport>
      <div class="user main">
      <TbaleList minHeight="450" :unfold="true" :tableData="tableData" @tabBtnOperation="eventOperation" :column="column"></TbaleList>
  ...
  export default {
    ...
    methods:{
      ...
      submit(data){
        //添加数据
        if(this.type==='add'){
          this.postData(data,this.$api.addOrder);
        }
         //编辑数据
        if(this.type==='edit'){
          this.postData(data,this.$api.editOrder);
        }
        //成交
      ...
      columns(){
        let c=[
          ...
          {prop:"state_text",name:"订单状态",minWidth:170,isShowType:"default"},
          {prop:"b_name",name:"车辆品牌",minWidth:170,isShowType:"default"},
          {prop:"v_version",name:"车辆型号",minWidth:170,isShowType:"default"},
          {prop:"price",name:"厂家指导价(元)",minWidth:170,isShowType:"default"},
          {prop:"deal_price",name:"成交价格(元)",minWidth:170,isShowType:"default"},
          ...
          {prop:"tions",name:"首付比例%",minWidth:170,isShowType:"default",isUnfold:true,isShow:true},
          {prop:"payment",name:"月供金额(元)",minWidth:170,isShowType:"default",isUnfold:true,isShow:true},
